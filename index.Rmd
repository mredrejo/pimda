...
#Loading packages
paquetes <- c("ggplot2","data.table","ggbreak","ggh4x","car","stringr","gdata","ggrepel","pzfx","gridExtra","kableExtra","corrplot","heatmaply","reshape2","patchwork","geomtextpath","plotly","Biostrings","dplyr")
...
The input DNA to be amplified is a mock metagenome made up of equal mass of purified genomes from 4 different bacteria, spanning Gram+ and Gram- species, and genomes with high and low GC, as detailed in the Table 1. Note that the used reference genomes were selected later on, using top hits of Blast searches perfomed the longest contigs from each assembly as query.
...
```{r profiling, fig.cap=capFig("Metagenomic samples reads profiling of amplified DNA by different MDA protocols"),fig.width=9, fig.height=5,fig.align = 'left'}
...
misas_plot <- ggplot(misas) + 
  ...
  geom_point(aes(x=Assembly, y=as.numeric(misas[,17])/4,fill=Assembly), shape=21, color="black", size=2,position=position_dodge(width=0.5)) +
...
We are indebted to Professor Margarita Salas for her longtime support and contributions to the discovery of piPolB and to the early development of this project.
This work was supported by a grants PGC2018-093723-A-100 and PID2021-123403NB-I00 funded by MCIN/AEI/10.13039/501100011033/FEDER A way to make Europe to M.R.R.
C. Egas laboratory was funded by the European Union´s Horizon 2020 Research and Innovation Program under Grant Agreement No 685474, the METAFLUIDICS project.
CDO was holder of a Fellowship from the Spanish Ministry of University FPU16/02665. 
